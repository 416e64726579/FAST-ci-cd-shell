...
TESTRUN=$(./create_test_run.sh $WALLARM_API_TOKEN)
...
(./wait_fast.sh $WALLARM_API_TOKEN $TEST_ID)
...
(./stop_recording.sh $WALLARM_API_TOKEN $TEST_RECORD_ID)
sleep 10

####
echo "Start running security tests"
(./start_security_test.sh $WALLARM_API_TOKEN $TEST_ID)
sleep 10

####
echo "Wait untill tests have passed"
(./wait_test_run.sh $WALLARM_API_TOKEN $TEST_ID)
